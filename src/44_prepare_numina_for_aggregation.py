...
def load_benchmark_data(bench_name):
    # ★ NuminaMath (学習データ作成用) の処理を追加
    if bench_name == "numina_train":
        print("Loading NuminaMath dataset (sampling 2000)...")
        dataset = load_dataset("AI-MO/NuminaMath-CoT", split="train")
        
        # ランダムサンプリング
        import random
        # データ数が2000未満の場合のエラー回避
        ...
        for item in dataset:
            standardized.append({
                ...
                "gold_answer": item["solution"] # Numinaはsolutionキー
            })
        return standardized

    # ★ 以下、既存のベンチマーク処理 (インデントを戻す)
...
# ==========================================
# ... (前半のimportやload_benchmark_dataは変更なし) ...
...
def run_generation(args):
    ...
    if args.num_samples >= 64: BATCH_SIZE = 4 
    elif args.num_samples >= 16: BATCH_SIZE = 16
    else: BATCH_SIZE = 32
    ...
    for bench_name in args.target_benchmarks:
        ...
        for seed in args.seeds:
            ...
            # --- 統計用カウンタ ---
            stats_total_paths = 0
            stats_correct_paths = 0
            stats_solved_problems = 0  # 少なくとも1つ正解できた問題数 (Pass@N)
            ...
            with tqdm(total=total_items, desc=f"    Generating {bench_name}") as pbar:
                for i in range(0, total_items, BATCH_SIZE):
                    ...
                    for j, output in enumerate(outputs):
                        ...
                        responses_data = []
                        problem_has_correct = False # この問題で正解が出たか
            ...
            # --- 結果表示 ---
            avg_acc = (stats_correct_paths / stats_total_paths * 100) if stats_total_paths > 0 else 0
            solve_rate = (stats_solved_problems / total_items * 100) if total_items > 0 else 0
            
            print(f"\n[Statistics] Seed {seed}")
            print(f"  Total Problems : {total_items}")
            print(f"  Total Paths    : {stats_total_paths}")
            print(f"  Avg Accuracy   : {avg_acc:.2f}% (Pass@1 approx)")
            print(f"  Solve Rate     : {solve_rate:.2f}% (Problems with at least 1 correct answer)")
