...
class VLLMStepBeamSearcher:
    ...
    def search(self, problem, beam_width=5, max_steps=20, num_candidates=5):
        beam = [(problem, 0.0, [])]
        MAX_TOKEN_LIMIT = 3500 # 安全マージン
        
        # ★重要: seed を指定して vLLM の再現性を確保
...
def main():
    args = parse_args()
    set_seed(args.seed) # PyTorch seed
    ...
    # 2. Generator Load
    print(f"Loading vLLM: {args.gen_model}")
    llm = LLM(
    ...
    for i, item in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        best_path_text, best_score, steps = searcher.search(
            problem, beam_width=args.beam_width, max_steps=args.max_steps, num_candidates=args.num_candidates
        )
        ...
        results.append({
            "problem": problem, "gold": gold_answer, "pred": pred_answer, "is_correct": is_correct
        })

    acc = correct_count / len(dataset)
    print(f"Final Accuracy (Seed {args.seed}): {acc:.2%}")
